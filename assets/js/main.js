    ...
    var imJs = {
        ...
        autoDetectTheme: function () {
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition((position) => {
                    ...
                    let today = new Date(position.timestamp)
                    let date = today.getFullYear()+'-'+('0' + (today.getMonth()+1)).slice(-2)+'-'+('0'+today.getDate()).slice(-2);
                    let storedSunsetDate = localStorage.getItem('storedSunsetDate')
                    if (storedSunsetDate==null || storedSunsetDate != date) {
                        let requesturl = `https://api.sunrise-sunset.org/json?lat=${lat}&lng=${lng}&date=today&formatted=0`
                        fetch(requesturl, {
                            method: 'GET'
                            ...
                            if (data.status == 'OK') {
                                ...
                                localStorage.setItem('storedSunsetDate',date)
                                localStorage.setItem('todaySunset', sunset)
                                localStorage.setItem('todaySunrise', sunrise)
                            }
                        })
                    } 
                    let sunrise = new Date(localStorage.getItem('todaySunrise')).getTime()
                    let sunset = new Date(localStorage.getItem('todaySunset')).getTime()
        ...
        vedioActivation: function (e) {
            $('#play-video').on('click', function (e) {
                ...
                $("#video-overlay").append('<iframe width="100%" height="100%" src="assets/videos/videos2.mp4" frameborder="0" allowfullscreen></iframe>');
            });
